...
《Schedule I》（v0.4.6f11 / Steam Build 24484559）的简体中文离线汉化源码仓库。
...
- 词条格式为每行一条 `原文=译文`，支持 `\\`、`\n`、`\r`、`\=` 转义。
- **不要添加 `Translations/deny_keys.txt` 黑名单中的词条**：这些是
  游戏自身会被代码读回的枚举成员（品质等级、按键名、设置项、等级、
  地区、角色创建分类、UI 弹窗应答等）与控制台命令，翻译后会破坏
  游戏逻辑（金钱消失、短信不显示、无法创建角色、联系人进度丢失、
  品质等级粘连等 bug 均源于此）。插件加载时最先读入黑名单并拒绝
  对应词条；含数字的标识符与颜色值等垃圾键也会被自动拒绝。
